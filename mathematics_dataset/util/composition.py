...
class SampleArgs(collections.namedtuple("SampleArgs", ("num_modules", "entropy"))):
    ...
    def split(self, count):
        ...
        module_counts = combinatorics.uniform_non_negative_integers_with_sum(count, num_child_modules)
...
class PreSampleArgs(
    collections.namedtuple("PreSampleArgs", ("min_modules", "max_modules", "min_entropy", "max_entropy"))
):
...
def _polynomial_entity(value, context):
    ...
    return Entity(
        ...
        description=lang.l.translate("Let {function} = {polynomial}."),
        handle=handle,
...
class Context(object):
    ...
    def mark_used(self, symbol):
        ...
        assert isinstance(symbol, str)
        if symbol in self._relation_symbols or symbol in self._self_symbols or symbol in self._child_symbols:
    ...
    def _sampler(self, value, sample_args):
        ...
        if not valid:
            raise ValueError("No valid samplers found: value={} sample_args={}".format(value, sample_args))
    ...
    def _value_entity(self, value, context):
        ...
        raise ValueError("Don't know how to handle value={} of type {}".format(value, type(value)))
    ...
    def sample(self, sample_args, values):
        ...
        def all_symbols():
            return self._relation_symbols.union(self._self_symbols).union(self._child_symbols)
        ...
        for value, child_sample_args in zip(values, sample_args_split):
            ...
            if child_sample_args.num_modules == 0:
                ...
                if not isinstance(entity, Entity):
                    raise RuntimeError("Expected entity, but got {} instead".format(entity))
                if not number.is_integer_or_rational_or_decimal(entity.value) and not isinstance(
                    entity.value, Polynomial
                ):
                    raise RuntimeError(
                        "sampler {} returned invalid value of type {}".format(sampler, type(entity.value))
                    )
                if (number.is_integer_or_rational_or_decimal(value) and entity.value != value) or (
                    isinstance(value, Polynomial) and not np.array_equal(entity.value.coefficients, value.coefficients)
                ):
                    raise RuntimeError(
                        "entity values differ, sampler={} wanted={} got={}".format(sampler, value, entity.value)
                    )
                if child_sample_args.num_modules != context.module_count:
                    raise RuntimeError(
                        "unused modules, value={} sample_args={} context.module_count={},"
                        " sampler={}".format(value, child_sample_args, context.module_count, sampler)
                    )
...
class Entity(object):
    ...
    def __init__(
        ...
        **description_kwargs
        ...
        child_description, description_kwargs = expand_entities(context, **description_kwargs)
        ...
        if isinstance(value, Polynomial) and expression is not None and polynomial_variables is None:
