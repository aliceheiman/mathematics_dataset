...
def train(entropy_fn):
    ...
    return _make_modules(entropy=entropy_fn(_ENTROPY_TRAIN), add_sub_entropy=entropy_fn(_ADD_SUB_ENTROPY_TRAIN))


def test():
    """Returns dict of testing modules."""
    return _make_modules(entropy=_ENTROPY_INTERPOLATE, add_sub_entropy=_ADD_SUB_ENTROPY_INTERPOLATE)
...
def _add_question_or_entity(context, p, q, is_question):
    ...
    if is_question:
        ...
        return composition.Entity(
            context=context, value=value, description=lang.l.translate("Let {self} = {p} + {q}."), p=p, q=q
        )
...
def div(value, sample_args, context=None):
    ...
    if is_question:
        ...
        return composition.Entity(
            context=context, value=answer, description=lang.l.translate("Let {self} be {p} divided by {q}."), p=p, q=q
        )
...
def nearest_integer_root(sample_args):
    ...
    return example.Problem(
        question=example.question(context, template, value=value, ordinal=ordinal, one_over_exponent=one_over_exponent),
        answer=answer,
    )
...
def _calculate(value, sample_args, context, add_sub, mul_div, length=None):
    ...
    if is_question:
        ...
        return composition.Entity(
            context=context, value=value, expression=op, description=lang.l.translate("Let {self} be {op}."), op=op
        )


def add_sub_multiple(value, sample_args, length=None):
    return _calculate(value, sample_args, None, add_sub=True, mul_div=False, length=length)


def mul_div_multiple(value, sample_args, length=None):
    return _calculate(value, sample_args, None, add_sub=False, mul_div=True, length=length)


@composition.module(number.is_integer_or_rational)
def mixed(value, sample_args, context=None, length=None):
    return _calculate(value, sample_args, context, add_sub=True, mul_div=True, length=length)
