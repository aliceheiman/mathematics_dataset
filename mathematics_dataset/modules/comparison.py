...
def _make_comparison_question(context, left, right):
    ...
    question = example.question(context, template, left=left, right=right, ops=ops)
...
def _entities_to_list(entities):
    ...
    for i, entity in enumerate(entities):
        ...
        values_template += str(entity.handle)
...
def _kth_biggest_list_question(context, entities, adjective, answer):
    ...
    question = example.question(
        context,
        lang.l.translate("What is the {adjective} value in {values_template}?"),
        adjective=adjective,
...
def _kth_biggest_multichoice_question(context, entities, adjective, answer):
    ...
    question = example.question(
        context,
        lang.l.translate("Which is the {adjective} value?") + choices_template,
        adjective=adjective,
        **entity_dict,
    )
...
def closest(sample_args, count=None):
    ...
    if display_multichoice:
        return _closest_multichoice_question(
            context=context, entities=entities, target=target, adjective=adjective, answer=answer
        )
    else:
...
def sort(sample_args, count=None):
    ...
    return example.Problem(
        question=example.question(
            context, template, direction=direction, unsorted_template=unsorted_template, **unsorted_dict
        ),
        answer=answer,
    )
