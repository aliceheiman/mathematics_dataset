...
def _make_modules(entropy, num_modules_composed):
    ...
    sample_args_composed = composition.PreSampleArgs(num_modules_composed[0], num_modules_composed[1], *entropy)

    for name, module in six.iteritems(fns):
        modules[name] = functools.partial(module, None, sample_args_pure)
        modules[name + "_composed"] = functools.partial(module, None, sample_args_composed)
...
def train(entropy_fn):
    ...
    return _make_modules(entropy=entropy_fn(_ENTROPY_TRAIN), num_modules_composed=_NUM_MODULES_COMPOSED)


def test():
    """Returns dict of testing modules."""
    return _make_modules(entropy=_ENTROPY_INTERPOLATE, num_modules_composed=_NUM_MODULES_COMPOSED)
...
def round_number(value, sample_args, context=None):
    ...
    if power > 0:
        ...
        if random.choice([False, True]):
            ...
            if lang.l.lang == "en":
                ...
                round_to = lang.l.translate(str(round_to))
        ...
        if power != -1:
            # Plural
            description = random.choice([lang.l.translate("{dps} decimal places"), lang.l.translate("{dps} dps")])
        dps = -power
        if random.choice([False, True]):
    ...
    return example.Problem(
        question=example.question(context, template, input=input_, description=description), answer=value
    )
...
def is_prime(value, sample_args, context=None):
    ...
    return example.Problem(
        question=example.question(
            context,
            lang.l.translate("Is {integer} {attribute}?"),
...
def is_factor(value, sample_args, context=None):
    ...
    answer = lang.l.translate(str(integer % maybe_factor == 0))

    return example.Problem(
        question=example.question(context, template, maybe_factor=maybe_factor, value=entity.expression_else_handle),
        answer=answer,
    )
...
def lcm(value, sample_args, context=None):
    ...
    if random.choice([False, True]):
        ...
        return example.Problem(
            question=example.question(
                context, template, adjective=adjective, p=p.expression_else_handle, q=q.expression_else_handle
            ),
            answer=answer,
        )
    else:
...
def gcd(value, sample_args, context=None):
    ...
    adjective = (
        random.choice([lang.l.translate("greatest"), lang.l.translate("highest")])
        + " "
        + lang.l.translate("common")
        + " "
...
def base_conversion(min_entropy, max_entropy):
    ...
    return example.Problem(
        question=example.question(
            context, template, from_str=display.NumberInBase(value, from_base), from_base=from_base, to_base=to_base
        ),
