...
def _make_modules(entropy):
    ...
    return {
        "coefficient_named": functools.partial(coefficient_named, None, sample_args_pure),
...
def coefficient_named(value, sample_args, context=None):
    ...
    return example.Problem(
        question=example.question(context, template, expression=expression, canonical=canonical, target=named_coeff),
        answer=value,
    )


_TEMPLATES = lang.l.parse(
    [
        "What is {composed}?",
...
def add(value, sample_args, context=None):
    ...
    c1, c2, fn1, fn2 = context.sample(
        sample_args, [c1, c2, composition.Polynomial(coeffs1), composition.Polynomial(coeffs2)]
    )
...
def expand(value, sample_args, context=None):
    ...
    template = random.choice([lang.l.translate("Expand {expression}.")])
...
def collect(value, sample_args, context=None):
    ...
    if is_question:
        ...
        return example.Problem(
            question=example.question(context, template, unsimplified=unsimplified), answer=simplified
        )
    else:
...
def compose(value, sample_args, context=None):
    ...
    entity_f, entity_g = context.sample(
        sample_args, [composition.Polynomial(coeffs_f), composition.Polynomial(coeffs_g)]
    )
...
def simplify_power(value, sample_args, context=None):
    ...
    return example.Problem(example.question(context, template, unsimplified=unsimplified, variable=variable), answer)
