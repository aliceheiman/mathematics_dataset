...
def _make_modules(entropy):
    ...
    return {
        "differentiate_composed": functools.partial(differentiate_univariate, None, sample_args_composed),
...
def _sample_integrand(coefficients, derivative_order, derivative_axis, entropy):
    ...
    pad = [(0, pad_amount if i == derivative_axis else 0) for i in range(coefficients.ndim)]
...
def _differentiate_polynomial(value, sample_args, context, num_variables):
    ...
    if is_question:
        ...
        return example.Problem(
            question=example.question(context, template, eq=polynomial, var=variable, nth=nth), answer=answer
        )
    else:
        ...
        return composition.Entity(
            ...
            description=lang.l.translate("Let {fn}({variables}) be the {nth} derivative of {eq}."),
