...
def _make_modules(entropy):
    ...
    return {
        ...
        "polynomial_roots_composed": functools.partial(polynomial_roots, None, sample_args_composed),
        "linear_1d": functools.partial(solve_linear_1d, None, sample_args_pure),
        "linear_1d_composed": functools.partial(solve_linear_1d, None, sample_args_composed),
        "linear_2d": functools.partial(solve_linear_2d, None, sample_args_pure),
        "linear_2d_composed": functools.partial(solve_linear_2d, None, sample_args_composed),
        # Sequences:
...
def test_extra():
    ...
    return {
        "polynomial_roots_big": functools.partial(polynomial_roots, None, sample_args_pure),
    }
...
def _solve_linear_system(degree, value, sample_args, context=None):
    ...
    if extra_solutions_needed > 0:
        ...
        solutions += [number.integer(solution_entropy, True) for solution_entropy in entropies]
    ...
    while True:
        equations = linear_system.linear_system(
            variables=variables, solutions=solutions, entropy=entropy, non_trivial_in=solution_index
        )
    ...
    if is_question:
        ...
        return composition.Entity(
            context=context,
            value=answer,
            description=lang.l.translate("Suppose {equations}."),
            handle=variable,
            equations=equations,
        )
...
class _PolynomialSequence(object):
    ...
    def __init__(self, variable, entropy, min_degree=1, max_degree=3):
        ...
        polynomial = polynomials.sample_with_small_evaluation(
            variable=self._variable, degree=self._degree, max_abs_input=self._degree + 2, entropy=entropy
        )
...
def sequence_nth_term(min_entropy, max_entropy):
    ...
    return example.Problem(
        question=example.question(context, template, variable=variable, sequence=sequence_sample), answer=answer
    )
