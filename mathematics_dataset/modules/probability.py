...
_GERUNDS = {
    lang.l.translate("pick"): lang.l.translate("picking"),
}
...
def _make_modules(is_train):
    ...
    return {
        "swr_p_sequence": functools.partial(swr_prob_sequence, is_train=is_train, sample_range=_SWR_SAMPLE_COUNT),
        "swr_p_level_set": functools.partial(swr_prob_level_set, is_train=is_train, sample_range=_SWR_SAMPLE_COUNT),
    }
...
def test_extra():
    ...
    return {
        ...
        "swr_p_level_set_more_samples": functools.partial(
            swr_prob_level_set, is_train=None, sample_range=_SWR_SAMPLE_COUNT_EXTRAPOLATE
        ),
    }
...
def _sequence_event(values, length, verb):
    ...
    event_description = lang.l.translate("the sequence is {sequence}").format(sequence=sequence)
...
def _level_set_event(values, length, verb):
    ...
    counts_and_values = [
        "{} {}".format(counts_dict[value], value) for value in shuffled_values if counts_dict[value] > 0
    ]
    ...
    event_description = template.format(counts_and_values=counts_and_values, verbing=verbing)
    return event, event_description
...
def _sample_letter_bag(is_train, min_total):
    ...
    while True:
        ...
        num_letters_total = random.randint(
            max(num_distinct_letters, min_total), min(_MAX_TOTAL_LETTERS, num_distinct_letters * _MAX_LETTER_REPEAT)
        )
    ...
    return LetterBag(
        weights=weights, random_variable=random_variable, letters_distinct=letters_distinct, bag_contents=bag_contents
    )
...
def _sample_without_replacement_probability_question(is_train, event_fn, sample_range):
    ...
    while True:
        ...
        event, event_description = event_fn(
            values=distinct_letters, length=space.n_samples, verb=lang.l.translate("pick")
        )
